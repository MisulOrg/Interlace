...
\input{misul_results_macros.tex}

\newcommand{\ModelName}{Interlace}
\input{misul_architecture_macros.tex}
...
Efficient language architectures must use their parameters for both stored
representations and useful computation. \ModelName{} combines bounded rational
features, content-selected attention, gated delta memory and shared depth in a
single backbone. The same parameters support autoregressive language,
synchronous input/thought/output streams and iterative candidate refinement
through an adaptation of Flow Reasoning Models. We train a
\num{29814640}-parameter instance from random initialization and compare it
with a \num{29921280}-parameter dense Transformer under a common BF16 joint
training recipe. \MisulArchitectureAbstract{} The packaged FP8 checkpoint answers
\MisulFPStreamHeld\% of held-out short programs and
\MisulFPStreamLong\% of longer programs through free-running streams.
The integrated refinement interface remains experimental, with
\MisulFPFlowEight\% exact held-out sequences after eight passes.
The results establish a runnable shared architecture and define the measured
capability and efficiency questions for its next stage of development.
...
\subsection{Synchronous streams and Flow reasoning}
...
\input{misul_architecture_timing.tex}
...
learned intermediate-state dependency on the evaluated task. They do not
establish unrestricted reasoning or instruction following.
...
The next phase targets architectural value: stronger held-out language and
structured-task performance per parameter, and less elapsed training time to
those outcomes. Controlled ablations should first separate rational gating,
selected access, delta memory and shared depth while keeping total state and
data budgets explicit. Multi-seed comparisons and broader corpora are needed
to determine whether the observed differences persist.

Reliable candidate refinement is a second milestone. It requires evidence that
additional passes improve exact solutions on fresh tasks while preserving
language and stream performance. A third milestone is scale: reproduce the
quality and efficiency results at larger model and data budgets with complete
training and inference accounting. These experiments provide measurable
criteria for investment in the architecture; scale-up benefits remain
hypotheses until those criteria are met.
...
\bibitem{monodratic} D. Todorov. \emph{Monodratic: A Reproducible Sparse Attention Mixer}. Technical manuscript and research software, 2026.
\end{thebibliography}
\end{document}
